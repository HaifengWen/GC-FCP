...
requested for the UAI rebuttal:
  ...
  * datasets: synthetic regression, CIFAR-10, CIFAR-100, PathMNIST, ImageNet-1K;
...
def target_to_int(y: Any) -> int:
    """Convert dataset targets from int / numpy / torch containers to int."""
...
def load_pathmnist_dataset(data_root: str, transform: Any):
    """Load PathMNIST validation+test splits, as in realdata_alpha_exp_table.py."""
    try:
        ...
        raise ImportError(
            "PathMNIST support requires the medmnist package. Install it with `pip install medmnist`."
        ) from e

    info = INFO["pathmnist"]
    DataClass = getattr(medmnist, info["python_class"])
    root = str(data_root or "./data")
    ...
    return ConcatDataset([val_ds, test_ds])


def build_fallback_pathmnist_cnn(n_classes: int = 9):
    """Fallback CNN compatible with a common PathMNIST SimpleCNN."""
    import torch.nn as nn

    class SimpleCNNFallback(nn.Module):
    ...
    return SimpleCNNFallback(n_classes=n_classes)


def load_pathmnist_model(device: str, model_path: str = "checkpoints/path_cnn.pt"):
    """Load the PathMNIST CNN checkpoint used by the paper-style experiments."""
    import torch

    try:
        from medmnist_train import SimpleCNN  # type: ignore
        model = SimpleCNN(n_classes=9)
    except Exception:
        model = build_fallback_pathmnist_cnn(n_classes=9)

    model_path = str(model_path or "")
    if not model_path:
        raise ValueError("PathMNIST requires --pathmnist_model_path pointing to a trained checkpoint.")
    if not Path(model_path).exists():
        raise FileNotFoundError(
            f"PathMNIST checkpoint not found: {model_path}. "
            "Use --pathmnist_model_path to provide the CNN checkpoint used in the paper."
        )
    ...
    try:
        ...
        if missing or unexpected:
            raise RuntimeError(
                "Could not strictly load PathMNIST checkpoint into the available SimpleCNN architecture. "
                f"Missing keys: {missing}; unexpected keys: {unexpected}. "
                "Ensure medmnist_train.py defining the original SimpleCNN is on PYTHONPATH."
            ) from e
    model.to(device).eval()
    return model
...
def load_classifier_model(dataset: str, device: str, imagenet_model: str = "resnet50", pathmnist_model_path: str = "checkpoints/path_cnn.pt"):
    import torch
    import torch.hub

    dataset = str(dataset).lower()
    model_id = str(imagenet_model).lower() if dataset == "imagenet" else dataset
    path_id = str(pathmnist_model_path) if dataset == "pathmnist" else ""
    key = (dataset, str(device), model_id, path_id)
    ...
    if dataset in {"cifar10", "cifar100"}:
        ...
        model = load_pathmnist_model(device=device, model_path=pathmnist_model_path)
    else:
...
def make_real_data(
    ...
    imagenet_cache: str = "",
    pathmnist_model_path: str = "checkpoints/path_cnn.pt",
    ambiguity_score: str = "margin",
    ...
    x_feature = (x_feature or "auto").lower()
    if x_feature == "auto":
        # Preserve the CIFAR-10 difficulty-group default while making PathMNIST
        # follow the paper's predicted-label grouping unless explicitly overridden.
        x_feature = "confidence" if dataset == "cifar10" else "pred_label"
    if x_feature not in {"pred_label", "confidence", "entropy"}:
        raise ValueError("x_feature must be one of {'auto', 'pred_label', 'confidence', 'entropy'}")
    ...
    if use_imagenet_cache:
        ...
        if dataset in {"cifar10", "cifar100"}:
            ...
            transform = transforms.ToTensor()
            ds = load_pathmnist_dataset(data_root=str(data_root_path), transform=transform)
            n_classes = 9
        elif dataset == "imagenet":
            import torchvision.models as models
    ...
    if use_imagenet_cache:
        ...
        prob_key = (
            ...
            str(pathmnist_model_path or "") if dataset == "pathmnist" else "",
        ...
        if cached is None:
            ...
            cal_labels_all = np.array([target_to_int(y) for _, y in cal_items], dtype=int)
            test_images = [x for x, _ in test_items]
            test_labels = np.array([target_to_int(y) for _, y in test_items], dtype=int)

            model = load_classifier_model(dataset, device, imagenet_model=imagenet_model, pathmnist_model_path=pathmnist_model_path)
    ...
    if dataset == "cifar100" and group_mode == "coarse_topr":
        ...
        if dataset in {"cifar10", "pathmnist"} and x_feature in {"confidence", "entropy"}:
            # Difficulty-style grouping from realdata_additional.py. For
            # PathMNIST this is optional; x_feature=auto resolves to pred_label
            # and follows the paper's predicted-label groups.
            ...
            if effective_group_mode == "disjoint_pred":
                ...
                if dataset == "cifar10":
                    ...
                    intervals = [
                        (a + 0.00 * (b - a), a + 0.45 * (b - a)),
                        (a + 0.25 * (b - a), a + 0.60 * (b - a)),
                        (a + 0.45 * (b - a), a + 0.75 * (b - a)),
                        (a + 0.60 * (b - a), a + 0.90 * (b - a)),
                        (a + 0.75 * (b - a), b),
                    ]
            ...
            group_meta = {
                ...
                "x_feature": x_feature,
                "intervals": intervals,
                "source": f"realdata_additional.py difficulty-style {dataset} grouping",
            }
        else:
            ...
            if dataset == "cifar10":
                ...
                # Paper-style PathMNIST groups from realdata_alpha_exp_table.py.
                if effective_group_mode == "disjoint_pred":
                    intervals = [(0, 2), (2, 4), (4, 6), (6, 8), (8, 9)]
                else:
                    intervals = [(0, 3), (1, 4), (2, 5), (3, 6), (4, 9)]
            else:
    ...
    if split_req == "auto":
        split_used = "dirichlet" if (dataset == "imagenet" or (dataset in {"cifar10", "pathmnist"} and K > n_classes)) else "label"
    else:
    ...
    if dataset in {"cifar10", "pathmnist"} and x_feature == "confidence":
        pred_or_scalar = np.max(probs_cal, axis=1).astype(float)
    elif dataset in {"cifar10", "pathmnist"} and x_feature == "entropy":
    ...
    return ExperimentData(
        ...
        metadata={
            ...
            "pathmnist_model_path": str(pathmnist_model_path or "") if dataset == "pathmnist" else "",
            **group_meta,
        },
    )
        ...
        for score in scores:
            ...
            if args.dataset == "synthetic":
                ...
                data = make_real_data(
                    ...
                    pathmnist_model_path=str(args.pathmnist_model_path) if getattr(args, "pathmnist_model_path", "") else "",
...
def build_parser() -> argparse.ArgumentParser:
    ...
    run.add_argument("--dataset", type=str, default="synthetic", choices=["synthetic", "cifar10", "cifar100", "pathmnist", "imagenet"])
    ...
    run.add_argument("--x_feature", type=str, default="auto", choices=["auto", "pred_label", "confidence", "entropy"],
                     help="Scalar feature used for CIFAR-10/PathMNIST group construction. auto uses confidence for CIFAR-10 and pred_label for PathMNIST, matching the paper setup for PathMNIST; pred_label recovers the paper-style predicted-label windows.")
    run.add_argument("--client_split", type=str, default="auto", choices=["auto", "label", "dirichlet"],
                     help="Client allocation for real data. auto keeps the original label split except CIFAR-10/PathMNIST with K > number of classes, where it switches to Dirichlet sample-level non-i.i.d. allocation to avoid empty clients.")
    ...
    run.add_argument("--pathmnist_model_path", type=str, default="checkpoints/path_cnn.pt", help="Path to the trained PathMNIST SimpleCNN checkpoint used for paper-style PathMNIST experiments.")
